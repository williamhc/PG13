...
{	
	...
	{
		int numPuzzles = db.getPuzzles().size();
		...
		db.savePuzzle(cr);
		assertEquals(numPuzzles + 1, db.getPuzzles().size());
		assertEquals(numPuzzles + 1, db.getSortedPuzzleIDs().size());
		numPuzzles = db.getPuzzles().size();
	...
	public void testAddingUniqueUsers()
	{
		...
		assertEquals(db.getSortedUserIDs().size(), numUsers);
		
		User u = new User(10l, "TestUser");
		db.saveUser(u);
		assertEquals(numUsers + 1, db.getUsers().size());
		assertEquals(numUsers + 1, db.getSortedUserIDs().size());
